...
func WriteEnrichmentReportToFile(ctx context.Context, req models.AuditReport, file *os.File) error {
	...
	query, err := getQueryForEnrichmentData(ctx, req)
	if err != nil {
		return err
	}
...
func getQueryForEnrichmentData(ctx context.Context, req models.AuditReport) (string, error) {
	...
	logging.GetLoggerWithContext(ctx).Info("query for enrichment data", zap.String("query", query), zap.String("request_id", req.Id.String()), zap.String("report_name", req.Name), zap.String("tenant_id", req.TenantId))
	return query, nil
}
