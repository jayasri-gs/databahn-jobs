...
func FetchDataTransformationReport(ctx context.Context, req models.AuditReport, wg *sync.WaitGroup, parallelismCntrl chan struct{}, failedRequests *[]models.FailedRequests, successAlerts *[]alerts_common.AlertBaseObjectV2, failedRequestMutex *sync.Mutex, successAlertsMutex *sync.Mutex) {
	var file *os.File
	...
	logging.GetLogger().Info("Fetching data transformation report", zap.String("request_id", req.Id.String()), zap.String("tenant_id", req.TenantId))
...
func getRowsAndColumnsFromAuditTable(pageSize int, offset int, query string) (*sql.Rows, []string, error) {
	rows, err := config.GetDB().Table("data_transformation").Limit(pageSize).Offset(offset).Where(query).Order("updated_at").Rows()
	if err != nil {
		return nil, nil, err
	}
