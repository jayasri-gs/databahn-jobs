...
func gatherDataAndWriteToFile(ctx context.Context, req models.AuditReport, startTime, endTime string, sources []string, file *os.File) error {
	...
	for {
		res, newSearchAfter, err := opensearch.SearchPaginated(ctx, opensearch.GetClient(), index, query, pageSize, searchAfter, []opensearch.Sort{{Field: "updated_at", Order: "asc"}, {Field: "_id", Order: "asc"}})
		if err != nil {
