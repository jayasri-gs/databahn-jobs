...
func WriteFleetReportToFile(ctx context.Context, req models.AuditReport, file *os.File) error {
	...
	query, err := getQueryForFleetData(ctx, req)
	if err != nil {
		return err
	}
...
func getReportAndWriteToFile(ctx context.Context, req models.AuditReport, query string, file *os.File) error {
	...
	for {
		...
		rows, columns, err := common.GetRowsAndColumnsByQueryFromTable("fleet", query, pageSize, offset)
		if err != nil {
...
func getQueryForFleetData(ctx context.Context, req models.AuditReport) (string, error) {
	...
	logging.GetLoggerWithContext(ctx).Info("query for fleet data", zap.String("query", query), zap.String("request_id", req.Id.String()), zap.String("report_name", req.Name), zap.String("tenant_id", req.TenantId))
	return query, nil
}
